	...
	while(1)
	{
		...
		switch (command)
		{
			...
			for(i=0;i<400;i++)
			{
				...
				if(uac_register()<1)
				{
					printf("uac_register error\n");
					break;
				}

				if(uac_key_nego()<1)
				{
					printf("uac_key_nego error\n");
					break;
				}
				if(user_type==NVR)
				{
				...
				if(uac_key_distribute("user2",&p2p_target)<1)
				{
					printf("uac_key_distribute error\n");
					break;
				}

				if(uac_token_exchange(&p2p_target,Auth)<1)
				{
					printf("uac_token_exchange error\n");
					break;
				}
				writekey(Securelinks.links[getSecureLinkNum(&Securelinks,p2p_target.username)].CK,KEY_LEN,1);
				if(uac_token_exchange(&p2p_target,Reauth)<1)
				{
					printf("uac_token_exchange error\n");
					break;
				}
				if(uac_token_exchange(&p2p_target,Byesession)<1)
				{
					printf("uac_token_exchange error\n");
					break;
				}
				if(uac_token_exchange(&p2p_target,Byelink)<1)
				{
					printf("uac_token_exchange error\n");
					break;
				}
